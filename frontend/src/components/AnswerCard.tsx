...
interface FastAPIAnswer {
  // Success path
  narrative?: string;
  key_finding?: string | { message?: string };
...
export default function AnswerCard({ raw }: Props) {
  ...
  const keyFinding =
    typeof answer.key_finding === "string"
      ? answer.key_finding
      : answer.key_finding?.message ?? null;

  const narrative = answer.narrative ?? keyFinding ?? "No insights returned.";
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Chart */}
      {answer.chart_spec && <MiniBarChart spec={answer.chart_spec} />}
